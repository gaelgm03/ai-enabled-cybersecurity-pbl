...
class ClassicDetector(BaseDetector):
    ...
    format patterns in source code. Supports Python and PHP.
    """

    # Relevant subtypes from unsafe_query.yaml for this detector
    LANGUAGE_SUBTYPES = ["string_concat", "unsafe_execute"]

    def __init__(self):
        """Initialize the detector with patterns for all supported languages."""
        # Load common patterns: error_based + union_based
        self._common_patterns: List[Dict[str, Any]] = []
        patterns_dir = Path(__file__).parent.parent / "patterns" / "common"

        for category in ["error_based", "union_based"]:
            file_path = patterns_dir / f"{category}.yaml"
            if file_path.exists():
                self._common_patterns.extend(self._load_yaml_patterns(file_path))
