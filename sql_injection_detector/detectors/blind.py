"""Blind SQL Injection Detector - Boolean-based and Time-based detection."""
...
class BlindDetector(BaseDetector):
    """Detector for blind SQL injection patterns.

    Detects boolean-based and time-based blind SQL injection patterns
    in source code. Supports Python and PHP.
    """

    # Relevant subtypes from unsafe_query.yaml for this detector
    LANGUAGE_SUBTYPES = ["boolean_based"]

    def __init__(self):
        """Initialize the detector with patterns for all supported languages."""
        # Load common patterns: blind.yaml
        self._common_patterns: List[Dict[str, Any]] = []
        patterns_dir = Path(__file__).parent.parent / "patterns" / "common"

        blind_file = patterns_dir / "blind.yaml"
        if blind_file.exists():
            self._common_patterns.extend(self._load_yaml_patterns(blind_file))
    ...
    def detect(self, file_path: str) -> List[Finding]:
        """Scan a file for blind SQL injection vulnerabilities.
    ...
    def detect_content(
        ...
        """Scan content for blind SQL injection vulnerabilities.
